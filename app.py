...
st.markdown(
    'Upload your master dataset ZIP file containing multiple DT CSV records'
    ' to analyze IEC overloading criteria, visualize graphs interactively,'
    ' select custom time slots, and export professional PDF and Excel reports.'
)

# UPDATED: Accept a single zip file containing all CSV batches
uploaded_zip = st.file_uploader(
    'Upload Master Dataset (ZIP file containing CSVs)', type=['zip']
)
...
if uploaded_zip:
  ...
  with st.spinner('Extracting and processing CSV files from ZIP...'):
    df = load_data_from_zip(uploaded_zip)

  if df.empty:
    st.error('No valid CSV files found inside the uploaded ZIP archive.')
  else:
  ...
  st.info('👆 Please upload your master ZIP file containing the CSV batches using the file uploader above.')
